...
void setup() {
  ...
  Serial.print("Input command, "); Serial.print("Dshot RPM, "); Serial.println("Optical RPM");
...
// ----- TRANSMISSION STATE ----- //
  ...
  if(TX){
    ...
    if(!iterate){
      ...
      if((int)input_signal == 100){ iterate = true; startTime = micros(); } // Initiate iteration @ throttle = 100
      
    }else if(iterate && forwards){ // Iterate forwards to 2000
      
      input_signal = (int)(100 + (float)(micros() - startTime)/10000.);
      if(input_signal > 2000){ forwards = !forwards; startTime = micros(); }
      
    }else if(iterate && !forwards){ // Iterate backwards to 100
      
      input_signal = (int)(2000 - (float)(micros() - startTime)/10000.);
      if(input_signal < 100){ forwards = !forwards; startTime = micros(); }
      
    }

    // Construct & send the signal
    ns2dshot_inv((int)input_signal, telemetry); // Remove wait time? Should go directly to listening
...
//        Serial.print("ERPM: "); Serial.println(eRPM);
        estimate_rpm();
        uint32_t rpm = averaged_rpm();
        Serial.print(input_signal); Serial.print(", "); Serial.print((long)eRPM / 7.); Serial.print(", "); Serial.println(rpm);
